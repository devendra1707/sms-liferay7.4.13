...
;(function ( $, window, document, undefined ) {
	...
	Plugin.prototype = {
		...
		_create_image_with_datasrc: function(datasrc, callback, file, dataurl) {
			...
			img.src = datasrc;
			//console.log("Data Source : "+img.src);
			img.onload = function() {
				if(dataurl) {
                    var canvas = document.createElement('canvas');
                    var ctx = canvas.getContext('2d');
		...
		_formsubmit: function() {
			if(!window.FormData) this.set_messagebox("Sorry, the FormData API is not supported!");
				...
				this.set_loading().delay(200).promise().done(function() {
					...
					//console.log("_this._image.src"+_this._image.src)
					if(_this._image) {
						...
						_this._theformdata.append("image", inputblob, _this._filename);
					}
					//send request
					...
					request.onload = function(e) {
						...
						_this.options.formSubmitted(this.responseText);
					};
    ...
    $.fn[pluginName] = function ( options ) {
        ...
        // treat this as a call to a public method.
            ...
            this.each(function () {
                ...
                if (/*options!=='destroy' && */instance instanceof Plugin && typeof instance[options] === 'function') {
